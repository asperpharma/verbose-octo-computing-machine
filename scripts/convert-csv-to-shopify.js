...
const INPUT_CSV = path.join(
  __dirname,
  "../../dataset_productss_2026-01-15_23-29-45-343 (1).csv",
);
...
function mapCategory(category, title, brand) {
  ...
  if (
    combined.includes("serum") || combined.includes("cream") ||
    combined.includes("moistur")
  ) {
    return "Skin Care";
  }
  if (
    combined.includes("shampoo") || combined.includes("hair") ||
    combined.includes("conditioner")
  ) {
    return "Hair Care";
  }
  if (
    combined.includes("perfume") || combined.includes("fragrance") ||
    combined.includes("eau de")
  ) {
    return "Fragrance";
  }
  if (
    combined.includes("makeup") || combined.includes("lipstick") ||
    combined.includes("mascara")
  ) {
    return "Makeup";
  }
  if (
    combined.includes("body") || combined.includes("lotion") ||
...
async function convertCSVToShopify() {
  ...
  const comparePriceCol = colIndex["variants/0/price/previous"] ||
    colIndex["compare_at_price"];
  ...
  for (let i = 1; i < lines.length; i++) {
    try {
      ...
      jsonProducts.push({
        ...
        compareAtPrice: comparePrice !== "0.00"
  ...
  console.log(`\n📝 Next Steps:`);
  console.log(
    `   1. Go to Shopify Admin: https://lovable-project-milns.myshopify.com/admin`,
  );
